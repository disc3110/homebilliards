...
| Database | **PostgreSQL** (Railway) | Quotes + manual metadata overrides — *not* the product catalog |
...
Remaining documents to produce, in build order:

| # | Document | Status | Blocked by |
|---|---|---|---|
| 1 | Design System (tokens from demo CSS) | Can start now | — |
| 2 | Component Inventory (from demo HTML) | Can start now | — |
| 3 | API Specification (OpenAPI for the NestJS BFF) | Can start now | — |
| 4 | Canonical Types + BigCommerce field mapping | Waiting | S5 (API access) |
| 5 | Quote Workflow Spec + email templates | Waiting | S3 |
| 6 | Search Integration Spec (Algolia schema, sync, synonyms) | Partially blocked | S6 |
| 7 | Analytics Implementation Spec (event → GA4/PostHog mapping) | Waiting | J7 |
| 8 | SEO indexing rules + 301 redirect map (extends SEO_SPEC.md) | Waiting | J1–J4 |
| 9 | Repo READMEs + dev setup | After scaffold | S5 |
| 10 | Deployment Runbook (Vercel + Railway) | After scaffold | — |
| 11 | Launch Checklist (DNS, redirects, analytics, QA) | Before launch | 8 |
